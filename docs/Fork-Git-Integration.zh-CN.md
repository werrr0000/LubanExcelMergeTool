...
正式发布版本：`Luban Excel Merge 1.1.0`。面向使用者的操作说明参见 [用户手册](LubanExcelMerge-User-Guide.zh-CN.md)。
...
- WPS/Excel 自动化接口不可用、重算超时或失败时不覆盖 MERGED。
- 可改用 `never` 保留来源缓存并标记下次打开时完整重算；保存后应使用 WPS 打开并保存一次，此前缓存状态会显示为“未验证”。
...
- 每个工作表独立解析 Luban 结构、选择主键、统计自动编辑并生成冲突。
...
- 不改变既有字段布局、主键布局和数据起始行的元数据变化会作为“元数据复核”项进入合并界面。
...
LOCAL 或 REMOTE 可以在任意位置新增字段列。单侧新增字段会自动写入 MERGED；双方新增不同字段时生成字段并集。MERGED 以 LOCAL 布局为基准，发生列位碰撞的 REMOTE 独有字段移动到首个安全空列。新字段数据按主键对齐，同名新字段的数据差异进入内容冲突，类型或元数据差异进入元数据复核。自动新增的元数据和数据可通过左下角自动编辑提示循环定位。

删除、重命名、重排或修改既有字段类型仍作为不安全结构变化阻止保存，也不支持改变元数据行或数据起始行。REMOTE 独有的新字段如果包含公式且必须移动列位，工具会阻止合并，避免未经解析地改写公式引用。数据区中首个非空单元格以 `##` 开头的注释/停用行不会参与主键校验或数据合并。
